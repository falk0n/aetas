...
#
# config-preprocess-recursive.py
# Configure the preprocess object in session state.
# In this case it is a RecursiveCharacterTextSplitter.
# You can configure chunk_size and chunk_overlap.
#
def recursive_text_splitter_preprocessor(docs):
...
st.write("# Configure RecursiveCharacterTextSplitter preprocessing")
...
new_kwargs["chunk_overlap"] = st.number_input(label="Chunk overlap", value=new_kwargs["chunk_overlap"], min_value=0)


if st.button("Set new preprocessing configuration"):
    st.session_state["preprocess"] = recursive_text_splitter_preprocessor
    st.session_state["preprocess_name"] = "RecursiveCharacterTextSplitter"
